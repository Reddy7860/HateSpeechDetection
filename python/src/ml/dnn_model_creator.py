from keras.models import Model # from keras.engine import Model
...
def create_final_model_with_concat_cnn(embedding_layers, model_descriptor:str):
    print("inside create_final_model_with_concat_cnn")
    print("model description : ",model_descriptor)
    ...
    print("target_grams : ",str(target_grams))

    submodels = []
    if ",so" in model_descriptor:
        print(",so in the model means skip layers only")
...
def create_skipped_conv1d_submodels(embedding_layers, cnn_ks, skip_layer_only:bool):
    print("inside create_skipped_conv1d_submodels")
    print("cnn_ks : ",cnn_ks)
    print("skip_layer_only : ",skip_layer_only)
    models=[]
    ...
    if cnn_ks<3:
        ...
        conv_layers.append(Conv1D(filters=100,
                          ...
                          padding='same', activation='relu'))
        add_skipped_conv1d_submodel_other_layers(conv_layers,embedding_layers,models)
    print("returning from create_skipped_conv1d_submodels ")
    return models


def add_skipped_conv1d_submodel_other_layers(conv_layers, embedding_layers,models:list):
    print("inside add_skipped_conv1d_submodel_other_layers")
    print(conv_layers)
...
def concat_embedding_layers(embedding_layers, big_model):
    print("writer mentioned its not working currently")
    print("inside concat_embedding_layers ")
    submodels = []
...
def generate_ks_and_masks(target_cnn_ks, skip):
    print("inside generate_ks_and_masks")
    # print("target_cnn_ks : ",target_cnn_ks)
    # print("skip : ",skip)
    masks=[]
